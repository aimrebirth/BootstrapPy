...
bootstrap_json = 'https://raw.githubusercontent.com/aimrebirth/Bootstrap/master/Bootstrap.json'
...
_7z     = '7za'
curl    = 'curl'
uvc     = 'UnrealVersionSelector'

_7z_ext = '.7z'
zip_ext = '.zip'

def load_data():
    s = download_to_variable(bootstrap_json)
    try:
        return json.loads(s, encoding = utf8)
    except:
        print('Json file: ' + bootstrap_json + ' has errors in its structure!')
        print('Please, report to the author!')
        exit(1)

def download_to_variable(url):
    p = subprocess.Popen([curl, '-L', '-k', '-s', url], stdout=subprocess.PIPE)
    out, _ = p.communicate()
